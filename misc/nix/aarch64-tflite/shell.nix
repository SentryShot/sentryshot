let
  # glibc 2.33
  pkgs = import (fetchTarball "https://github.com/NixOS/nixpkgs/archive/668e3761801bf72d78d40b28c87e80fd038974d2.tar.gz"){};
  #pkgs = import (fetchTarball "https://github.com/NixOS/nixpkgs/archive/3c15feef7770eb5500a4b8792623e2d6f598c9c1.tar.gz"){};
  bazel_5 = (
    pkgs.callPackage (
    ...
    pkgs.callPackage (
      ...
      let
        ...
        bazelDepsSha256ByBuildAndHost = {
          x86_64-linux = {
            x86_64-linux = "sha256-UzSuApkcgsgMYiFwA+HVDR+TdWBgZAQpSN+v/heYXwA=";
          ...
          aarch64-linux = {
            ...
            aarch64-linux = "sha256-inzXoJ0UcxuV4DqOVngT7Lv0zd6eV5of3Hw9XnHGH8I=";
          };
        };
      ...
      buildBazelPackage rec {
        ...
        buildAttrs = {
          installPhase = ''
            mkdir -p $out
            cp ./bazel-bin/tensorflow/lite/c/libtensorflowlite_c.so $out/
          '';
        };
  ...
  {} );
in pkgs.mkShell {
  buildInputs = [ tflite pkgs.pkg-config ];
  TFLITELIB = "${tflite}";
}
