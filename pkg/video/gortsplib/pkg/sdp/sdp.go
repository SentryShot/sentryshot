...
// Errors.
var (
	ErrInvalidSyntax       = errors.New("sdp: invalid syntax")
	ErrInvalidNumericValue = errors.New("sdp: invalid numeric value")
	ErrInvalidValue        = errors.New("sdp: invalid value")
	ErrInvalidPortValue    = errors.New("sdp: invalid port value")
...
func parsePort(value string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%w `%v`", ErrInvalidPortValue, port)
	}

	if port < 0 || port > 65536 {
		return 0, fmt.Errorf("%w -- out of range `%v`", ErrInvalidPortValue, port)
	}
...
func (s *SessionDescription) unmarshalSessionName(value string) {
...
func (s *SessionDescription) unmarshalOrigin(value string) error {
	...
	fields := func() []string {
		...
		return append(
			[]string{strings.Join(values[0:len(values)-5], " ")},
			values[len(values)-5:]...,
		)
	}()

	if len(fields) != 6 {
		return fmt.Errorf("%w `o=%v`", ErrInvalidSyntax, fields)
	}
	...
	if err != nil {
		return fmt.Errorf("%w `%v`", ErrInvalidNumericValue, fields[1])
	}

	sessionVersion, err := strconv.ParseUint(fields[2], 10, 64)
	if err != nil {
		return fmt.Errorf("%w `%v`", ErrInvalidNumericValue, fields[2])
	}

	// Set according to currently registered with IANA
	// https://tools.ietf.org/html/rfc4566#section-8.2.6
	if i := indexOf(fields[3], []string{"IN"}); i == -1 {
		return fmt.Errorf("%w `%v`", ErrInvalidValue, fields[3])
	}
...
func (s *SessionDescription) unmarshalSessionInformation(value string) {
...
func (s *SessionDescription) unmarshalEmail(value string) {
	emailAddress := psdp.EmailAddress(value)
	s.EmailAddress = &emailAddress
}

func (s *SessionDescription) unmarshalPhone(value string) {
...
func unmarshalConnectionInformation(value string) (*psdp.ConnectionInformation, error) {
	...
	if len(fields) < 2 {
		return nil, fmt.Errorf("%w `c=%v`", ErrInvalidSyntax, fields)
	}

	// Set according to currently registered with IANA
	// https://tools.ietf.org/html/rfc4566#section-8.2.6
	if i := indexOf(fields[0], []string{"IN"}); i == -1 {
		return nil, fmt.Errorf("%w `%v`", ErrInvalidValue, fields[0])
	}

	// Set according to currently registered with IANA
	// https://tools.ietf.org/html/rfc4566#section-8.2.7
	if i := indexOf(fields[1], []string{"IP4", "IP6"}); i == -1 {
		return nil, fmt.Errorf("%w `%v`", ErrInvalidValue, fields[1])
	}
...
func (s *SessionDescription) unmarshalSessionConnectionInformation(value string) error {
	...
	if err != nil {
		return fmt.Errorf("%w `c=%v`", ErrInvalidSyntax, value)
	}
	return nil
}
...
func unmarshalBandwidth(value string) (*psdp.Bandwidth, error) {
	...
	if len(parts) != 2 {
		return nil, fmt.Errorf("%w `b=%v`", ErrInvalidValue, parts)
	}
	...
	if experimental {
		...
		return nil, fmt.Errorf("%w `%v`", ErrInvalidValue, parts[0])
	}

	bandwidth, err := strconv.ParseUint(parts[1], 10, 64)
	if err != nil {
		return nil, fmt.Errorf("%w `%v`", ErrInvalidNumericValue, parts[1])
	}
...
func (s *SessionDescription) unmarshalSessionBandwidth(value string) error {
	...
	if err != nil {
		return fmt.Errorf("%w `b=%v`", ErrInvalidValue, value)
	}
...
func (s *SessionDescription) unmarshalTimeZones(value string) error {
	...
	fields := strings.Fields(value)
	if len(fields)%2 != 0 {
		return fmt.Errorf("%w `t=%v`", ErrInvalidSyntax, fields)
	}
	...
	for i := 0; i < len(fields); i += 2 {
		...
		if err != nil {
			return fmt.Errorf("%w `%v`", ErrInvalidValue, fields)
		}
...
func (s *SessionDescription) unmarshalSessionEncryptionKey(value string) {
	encryptionKey := psdp.EncryptionKey(value)
	s.EncryptionKey = &encryptionKey
}

func (s *SessionDescription) unmarshalSessionAttribute(value string) {
...
func (s *SessionDescription) unmarshalRepeatTimes(value string) error {
	fields := strings.Fields(value)
	if len(fields) < 3 {
		return fmt.Errorf("%w `r=%v`", ErrInvalidSyntax, fields)
	}
...
func (s *SessionDescription) unmarshalMediaDescription(value string) error {
	fields := strings.Fields(value)
	if len(fields) < 4 {
		return fmt.Errorf("%w `m=%v`", ErrInvalidSyntax, fields)
	}
	...
	if i := indexOf(fields[0], []string{"audio", "video", "text", "application", "message"}); i == -1 {
		return fmt.Errorf("%w `%v`", ErrInvalidValue, fields[0])
	}
	...
	if err != nil {
		return fmt.Errorf("%w `%v`", ErrInvalidPortValue, parts[0])
	}

	if len(parts) > 1 {
		portRange, err := strconv.Atoi(parts[1])
		if err != nil {
			return fmt.Errorf("%w `%v`", ErrInvalidValue, parts)
		}
...
func (s *SessionDescription) unmarshalMediaTitle(value string) {
...
func (s *SessionDescription) unmarshalMediaBandwidth(value string) error {
	...
	bandwidth, err := unmarshalBandwidth(value)
	if err != nil {
		return fmt.Errorf("%w `b=%v`", ErrInvalidSyntax, value)
	}
	latestMediaDesc.Bandwidth = append(latestMediaDesc.Bandwidth, *bandwidth)
	return nil
}

func (s *SessionDescription) unmarshalMediaEncryptionKey(value string) {
	latestMediaDesc := s.MediaDescriptions[len(s.MediaDescriptions)-1]
	encryptionKey := psdp.EncryptionKey(value)
	latestMediaDesc.EncryptionKey = &encryptionKey
}

func (s *SessionDescription) unmarshalMediaAttribute(value string) {
	i := strings.IndexRune(value, ':')
	var a psdp.Attribute
	if i > 0 {
		a = psdp.NewAttribute(value[:i], value[i+1:])
...
// Errors.
var (
	ErrSestionInvalidLine = errors.New("invalid line")
	ErrSessionInvalidKey  = errors.New("invalid key")
)

// Unmarshal decodes a SessionDescription.
// This is rewritten from scratch to guarantee compatibility with most RTSP
// implementations.
func (s *SessionDescription) Unmarshal(byts []byte) error { //nolint:funlen,gocognit
	str := string(byts)
	...
	for _, line := range strings.Split(strings.ReplaceAll(str, "\r", ""), "\n") {
		...
		if len(line) < 2 || line[1] != '=' {
			return fmt.Errorf("%w: (%s)", ErrSestionInvalidLine, line)
		}
		...
		case stateInitial:
			...
			default:
				return fmt.Errorf("%w: %c (%s)", ErrSessionInvalidKey, key, line)
			}

		case stateSession:
			if err := s.stateSession(key, val, line, &state); err != nil {
				return err
			}
		...
		case stateMedia:
			...
			case 'k':
				s.unmarshalMediaEncryptionKey(val)

			case 'a':
				s.unmarshalMediaAttribute(val)
...
func (s *SessionDescription) stateSession(key byte, val string, line string, state *stateVal) error { //nolint:funlen
	switch key {
	case 'o':
	...
	case 's':
		s.unmarshalSessionName(val)

	case 'i':
		s.unmarshalSessionInformation(val)
	...
	case 'e':
		s.unmarshalEmail(val)

	case 'p':
		s.unmarshalPhone(val)
	...
	case 'k':
		s.unmarshalSessionEncryptionKey(val)

	case 'a':
		s.unmarshalSessionAttribute(val)
