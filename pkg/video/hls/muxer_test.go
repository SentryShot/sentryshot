...
func TestMuxerVideoAudio(t *testing.T) {
	...
	dem := mpegts.NewDemuxer(context.Background(), m.Segment(ma[2]),
...
func TestMuxerVideoOnly(t *testing.T) {
	...
	m, err := NewMuxer(3, 1*time.Second, 50*1024*1024, nil, videoTrack, nil)
	require.NoError(t, err)
	defer m.Close()
...
func TestMuxerMaxSegmentSize(t *testing.T) {
	...
	m, err := NewMuxer(3, 1*time.Second, 0, nil, videoTrack, nil)
	require.NoError(t, err)
	defer m.Close()
