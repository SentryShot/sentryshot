...
// https://www.etsi.org/deliver/etsi_en/300400_300499/300468/01.15.01_60/en_300468v011501p.pdf
type DescriptorContent []*DescriptorContentItem
...
func newDescriptorContent(r *bitio.CountReader, offsetEnd int64) (DescriptorContent, error) {
	...
	for r.BitsCount/8 < offsetEnd {
		d = append(d, &DescriptorContentItem{
...
type DescriptorMaximumBitrate uint32 // In bytes/second. 22 bits.
...
func newDescriptorParentalRating(r *bitio.CountReader, offsetEnd int64) (DescriptorParentalRating, error) {
	...
	for r.BitsCount/8 < offsetEnd {
		...
		d = append(d, &DescriptorParentalRatingItem{
			CountryCode: country,
			Rating:      rating,
		})
	}
...
type DescriptorPrivateDataSpecifier uint32

func newDescriptorPrivateDataSpecifier(r *bitio.CountReader) (DescriptorPrivateDataSpecifier, error) {
	specifier := r.TryReadBits(32)
	return DescriptorPrivateDataSpecifier(specifier), r.TryError
}
...
// Chapter: 6.2.43 | Link:
// https://www.etsi.org/deliver/etsi_en/300400_300499/300468/01.15.01_60/en_300468v011501p.pdf
type DescriptorTeletext []*DescriptorTeletextItem
