...
// ErrKeyValApexesNotClosed apexes not closed.
var ErrKeyValApexesNotClosed = errors.New("apexes not closed")
...
func readValue(origstr string, str string, separator byte) (string, string, error) {
	if len(str) > 0 && str[0] == '"' {
		...
		for {
			if i >= len(str) {
				return "", "", fmt.Errorf("%w (%v)", ErrKeyValApexesNotClosed, origstr)
			}
