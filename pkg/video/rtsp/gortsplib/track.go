...
import (
	...
	"nvr/pkg/video/rtsp/gortsplib/pkg/sdp"
	"strconv"
	"strings"
...
func (t *Track) ClockRate() (int, error) {
	...
	for _, a := range t.Media.Attributes {
		if a.Key == "rtpmap" {
			...
			if len(tmp) < 2 {
				return 0, fmt.Errorf("%w (%v)", ErrTrackRTPmapInvalid, a.Value)
			}
	...
	return 0, ErrTrackRTPmapMissing
}
...
func ReadTracks(byts []byte) (Tracks, error) {
	...
	for i, track := range tracks {
		...
		if err != nil {
			return nil, fmt.Errorf("unable to get clock rate of track %d: %w", i, err)
		}
	}
