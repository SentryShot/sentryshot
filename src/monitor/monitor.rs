...
impl MonitorManagerState {
    async fn new(req: InitializeRequest) -> Result<Self, InitializeMonitorManagerError> {
        ...
        let mut state = Self {
            ...
            recdb_main: req.recdb_main,
            recdb_sub: req.recdb_sub,
            eventdb: req.eventdb,
            logger: req.logger,
    ...
    async fn new_test_manager() -> (
        ...
        manager
            .initialize(
                ...
                tracker.token(),
                config_dir.clone(),
                Arc::new(new_test_recdb(&temp_dir.path().join("main")).await),
                Arc::new(new_test_recdb(&temp_dir.path().join("sub")).await),
