			...
			for (const category of Object.values(categories)) {
				htmlNav += /* HTML */ `
					<li
						...
						style="
							padding: calc(var(--spacing) * 0.34) calc(var(--spacing) * 3.4) calc(var(--spacing) * 0.34) calc(var(--spacing) * 1.35);
				...
				<div class="settings-category" style="width: 100vw;">
					<div class="settings-menubar js-settings-menubar">
						...
						<span
							...
							style="
								margin: auto;
								margin-right: calc(var(--spacing) * 5.4);
								font-size: 2.7rem;
							"
						>${title}</span>
					</div>
	...
	const renderMonitorList = (monitors) => {
		...
		html += /* HTML */ `
			<button
				...
				style="
					margin: calc(var(--spacing) * 0.68) auto calc(var(--spacing) * 0.68) calc(var(--spacing) * 1);
